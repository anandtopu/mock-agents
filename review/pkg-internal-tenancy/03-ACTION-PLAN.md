...
- [ ] **Tenant-CRUD cross-tenant access** — `X-TN-001` · S1 · effort:L (design) — a per-tenant admin can list **all** tenants, delete **any** tenant (cascade-wiping its keys), and create tenants, because `GET/POST /api/v1/tenants` + `DELETE /api/v1/tenants/{id}` are gated by `RequireRole(admin)` only, with no ownership check and no super-admin tier. **Decide one:**
