...
| ID | Eff | Site | Change |
|----|-----|------|--------|
| ~~PERF-12~~ ⛔ declined | S | `tenancy/store.go`, `audit/store.go` | Prepare the hot `Resolve` prefix-SELECT + `last_used` UPDATE and the audit `Append` INSERT once (`db.Prepare`) instead of re-parsing every call. **Declined 2026-06-03 — premise doesn't hold:** every `Resolve` DB hit runs `bcrypt.CompareHashAndPassword` (~36 ms cold, per `auth_cache.go`) — the real compare on a hit, the **dummy** compare on a miss (the X-TN-002 timing defense). The cited `auth.denied` flood therefore costs ~36 ms of bcrypt per invalid key; a prepared SELECT saves single-digit-µs of SQL parse — **~4 orders of magnitude** smaller. The audit `Append` INSERT runs *synchronously after* that same bcrypt (`RecordHTTP`→`Append`), so it's behind the same wall. Valid keys are served from the auth cache without touching the DB at all. Adding prepared-statement lifecycle (struct fields + prepare-at-construction + `Close`) to two stores buys no measurable gain. |
...
| ~~PERF-18~~ ⛔ declined | M | `server/log_broadcaster.go` `Publish` | Copy-on-write `atomic.Pointer[[]*LogSubscription]` snapshot so `Publish` ranges a lock-free slice (today it holds one `Mutex` across the whole subscriber iteration on the async write loop). Low impact at the single-process target. **Declined 2026-06-03 — the mutex is load-bearing:** `Publish` and `Cancel` are currently mutually exclusive under `b.mu`, which is precisely what makes `Cancel`'s `close(sub.ch)` safe against `Publish`'s `sub.ch <- entry`. A lock-free COW `Publish` could range an old snapshot still containing a subscription *while* `Cancel` closes that subscription's channel → **send-on-closed-channel panic**. Making COW correct needs a channel-lifecycle redesign (don't-close + separate done signal, or per-sub send/close lock). Trading the proven-safe mutex for a panic hazard to win a lock-free publish the survey itself rates "low impact" is a bad trade. |
...
10. ~~**PERF-10**~~ **✅ done 2026-06-03** (moved the SELECT + per-key bcrypt out
    ...
    That closes the entire prioritized P1/P2 backlog (PERF-01..11).

**P3 sweep (2026-06-03).** All ten P3 items were worked or adjudicated:
- **Done:** PERF-15 (pre-size anthropic convert), PERF-16 (typed error
  envelopes), PERF-17 (hand-rolled UUID hex, −54 % ns/7→1 allocs),
  PERF-19 (token-count off flattened messages), PERF-20 (pre-size log
  page), PERF-21 (ReadHeaderTimeout), PERF-13 *index* (composite
  `(tenant_id, id DESC)`), PERF-14 *inline* (≤2 tool calls run inline,
  −57 % ns/10→6 allocs).
- **Partial, rest deferred with rationale:** PERF-13 keyset pagination
  (public pagination-contract change → handler + GUI), PERF-14
  `indexTools` cache (registry-side invalidation risk > one map build).
- **Declined after premise check:** PERF-12 (every `Resolve` DB hit is
  bcrypt-bound ~36 ms; SQL-parse caching is ~4 orders of magnitude
  smaller), PERF-18 (the `Publish` mutex is load-bearing for
  channel-close safety; COW would introduce a send-on-closed-channel
  panic for a "low impact" win).
